...
//!
//! These symbols only exist in binaries linked by cosmocc, so this crate is only
//! meant for projects generated by rust-ape.
...
unsafe extern "C" {
    ...
    #[link_name = "__hostos"]
    unsafe static HOSTOS: c_int;

    unsafe fn IsQemuUser() -> c_int;
}

/// The host we're currently running on.
...
pub fn cpu_count() -> i32 {
    // This used to call __get_cpu_count, but upstream renamed that private
    // symbol, so use sysconf instead, which is stable.
    unsafe { sysconf(84 /* _SC_NPROCESSORS_ONLN */) as i32 }
}
...
/// loader (a small stub) instead of the program that's running.
