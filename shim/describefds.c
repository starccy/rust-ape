...
╚─────────────────────────────────────────────────────────────────────────────*/
// [rust-ape] __describe_fds, replacing cosmo 4.0.2's libc.a(describefds.o)
// for one addition, marked [rust-ape] below: a descriptor whose handle
// cannot be duplicated for the child is reported on stderr with the
// descriptor's table entry and the win32 error. Such a failure means the
// descriptor table and the handle table disagree, which execve otherwise
// reports as a bare EBADF with nothing to trace it back to.
//
...
#include "libc/intrin/kprintf.h"  // [rust-ape]
...
textwindows char *__describe_fds(const struct Fd *fds, size_t fdslen,
  ...
  for (int fd = 0; fd < fdslen; ++fd) {
    ...
    if (!DuplicateHandle(GetCurrentProcess(), f->handle, hCreatorProcess,
      ...
      // [rust-ape] leave a trace of which entry the handle table disowned
      // fixme: print this debug information for catching bugs
      kprintf("[rust-ape] describe_fds: fd=%d kind=%d flags=%#x mode=%#o "
              "handle=%ld cursor=%p creator=%ld win32=%u\n",
              fd, f->kind, f->flags, f->mode, f->handle, f->cursor,
              hCreatorProcess, GetLastError());
      __winerr();
      goto OnFailure;
    }
    ...
    if (f->cursor) {
      ...
      if (!DuplicateHandle(GetCurrentProcess(), map->hand, hCreatorProcess,
        ...
        kprintf("[rust-ape] describe_fds: fd=%d cursor map handle=%ld "
                "win32=%u\n", fd, map->hand, GetLastError());  // [rust-ape]
        __winerr();
        goto OnFailure;
      }
      handles[hi++] = shand;
