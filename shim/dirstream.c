...
╚─────────────────────────────────────────────────────────────────────────────*/
// [rust-ape] dirstream, replacing cosmo 4.0.2's libc.a(dirstream.o). One
// change, marked [rust-ape] below: on NT a directory symlink is reported
// as DT_LNK. Upstream tests the directory attribute before the reparse
// attribute, so readdir called a symlink to a directory a directory and
// any tree walker that trusts d_type followed the link. master has the
// order this file uses.
//
...
static textwindows uint8_t GetNtDirentType(struct NtWin32FindData *w) {
  switch (w->dwFileType) {
    ...
    case kNtFileTypePipe:
      return DT_FIFO;
    default:
      // [rust-ape] a directory symlink carries both attributes and is a
      // link first, which is the order master uses
      if (w->dwFileAttributes & kNtFileAttributeReparsePoint) {
        return DT_LNK;
      } else if (w->dwFileAttributes & kNtFileAttributeDirectory) {
        return DT_DIR;
      } else {
        return DT_REG;
      }
