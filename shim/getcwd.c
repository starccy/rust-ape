...
╚─────────────────────────────────────────────────────────────────────────────*/
// [rust-ape] __getcwd, replacing cosmo 4.0.2's libc.a(getcwd.greg.o) for
// one addition, marked [rust-ape] below: a UNC cwd is reported to
// mkntpath.c's share registry. cosmo spells \\server\share as
// //server/share, and unix path code collapses that to /server/share,
// which is how a program working under a UNC cwd ends up asking for
// paths NT can only resolve once it knows they name that share. The
// cwd the program starts with is the one UNC path it never passes
// through __mkntpath itself, so it is registered here.

// Everything else is a faithful copy of upstream. Compiled with -D_COSMO_SOURCE
...
int __ape_shim_unc_cwd(char *, size_t);  // [rust-ape] shim/uncshare.c
void __ape_shim_unc_note(const char *);
...
static dontinline textwindows int sys_getcwd_nt(char *buf, size_t size) {
  ...
  // [rust-ape] a cwd inside the materialized "//server" tree is reported
  // as that server; any real share seen is remembered for mkntpath.c
  int m = __ape_shim_unc_cwd(buf, size);
  if (m)
    n = m;
  __ape_shim_unc_note(buf);
