...
╚─────────────────────────────────────────────────────────────────────────────*/
// [rust-ape] sys_readlinkat_nt, replacing cosmo 4.0.2's
// libc.a(readlinkat-nt.o). Two fixes, both marked [rust-ape] below:
// readlink() of a path that exists but is not a reparse point fails
// with EINVAL (the answer musl-realpath's walk keys on; upstream left
// errno untouched there), and the structural trailing backslash of a
// drive root is kept (stripping it turns \\?\C:\ into the
// drive-relative "C:"). Everything else is a faithful copy. Compiled
// with -D_COSMO_SOURCE by the linker wrapper. Revisit on toolchain
// upgrade.
...
static textwindows ssize_t sys_readlinkat_nt_impl(int dirfd, const char *path,
  ...
  // [rust-ape] a trailing backslash after a drive colon is structural,
  // not a user-supplied "assert this is a directory": stripping it
  // turns \\?\C:\ (the root directory) into \\?\C: (the current
  // directory on that drive). A drive root can never be a symlink, so
  // leave its slash alone; the directory assertion is vacuously true
  // for it.
  bool must_be_directory =
      len > 1 && path16[len - 1] == '\\' && path16[len - 2] != ':';
...
#pragma GCC pop_options
  ...
  if ((h = CreateFile(path16, kNtFileReadAttributes,
    ...
    if (DeviceIoControl(h, kNtFsctlGetReparsePoint, 0, 0, rdb, mem, &bc, 0)) {
      ...
      // [rust-ape] a plain file or directory fails
      // FSCTL_GET_REPARSE_POINT with ERROR_NOT_A_REPARSE_POINT, which
      // POSIX spells EINVAL; upstream returned -1 with errno untouched
      // here.
      if (GetLastError() == kNtErrorNotAReparsePoint) {
        rc = einval();
      } else {
        rc = __winerr();
      }
